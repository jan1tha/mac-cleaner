...
const CATEGORIES = [
  ...
  { id: 'wso2', title: 'WSO2 Update tool  (~/.wso2-updates)', risk: 'safe', last: true, lazy: true,
    desc: 'Data the WSO2 Update tool keeps, as its two subfolders. Deleting backup means you can no longer roll back an update on your local servers; deleting updates makes the next local-binary update take longer, since the payloads re-download. Sized in the background so the scan stays fast.',
    collect: () => existing([j('.wso2-updates/backup'), j('.wso2-updates/updates')]) },
...
async function collectScan(emit) {
  ...
  const built = await Promise.all(catDefs.map(async ({ cat, items }) => {
    ...
    if (cat.lazy) {   // don't walk it now — the client measures it in the background
      group = {
        id: cat.id, title: cat.title, risk: cat.risk, desc: cat.desc, last: !!cat.last, lazy: true, totalKb: null,
      ...
      group = {
        id: cat.id, title: cat.title, risk: cat.risk, desc: cat.desc, last: !!cat.last,
  ...
  // safe groups first, then review; within a tier, largest first — except groups
  // flagged `last`, which are pinned to the end of their tier regardless of size.
  const rank = r => (r === 'safe' ? 0 : 1);
  built.sort((a, b) =>
    rank(a.risk) - rank(b.risk) ||
    (a.last ? 1 : 0) - (b.last ? 1 : 0) ||
    (b.totalKb || 0) - (a.totalKb || 0));
