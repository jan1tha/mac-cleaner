...
const CATEGORIES = [
  ...
  { id: 'wso2-backups', title: 'WSO2 update backups  (~/.wso2-updates/backup)', risk: 'safe', lazy: true,
    desc: 'Snapshots the WSO2 Update tool saves before patching a product — one folder per product/version. The tool writes a fresh backup before each patch, so old ones are safe to clear once you no longer need to roll those patches back. (Sized in the background so the scan stays fast.)',
    collect: () => listChildren(j('.wso2-updates/backup')).filter(p => path.basename(p) !== '.DS_Store') },
  { id: 'wso2-updates', title: 'WSO2 downloaded updates  (~/.wso2-updates/updates)', risk: 'review', lazy: true,
    desc: 'Update payloads the WSO2 Update tool has downloaded, one folder per version transition. Usually the largest WSO2 item — but the tool re-downloads whatever it needs on the next update (large, needs network), so clear only if you can re-fetch. (Sized in the background so the scan stays fast.)',
    collect: () => listChildren(j('.wso2-updates/updates')).filter(p => path.basename(p) !== '.DS_Store') },
