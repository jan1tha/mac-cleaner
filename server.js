...
const CATEGORIES = [
  ...
  { id: 'dev-caches', title: 'Developer build caches', risk: 'safe',
    desc: 'Package-manager and build caches (npm, Yarn, pnpm, Gradle, Maven, Cargo, Go, CocoaPods, pip) and Xcode DerivedData. Re-downloaded or rebuilt on demand.',
    collect: () => existing([
      j('.npm/_cacache'), j('.yarn/cache'), j('Library/Caches/Yarn'),
      j('Library/pnpm/store'), j('.pnpm-store'),
      j('.gradle/caches'), j('.gradle/wrapper'),
      j('.m2/repository'), j('.cargo/registry'), j('go/pkg/mod'),
